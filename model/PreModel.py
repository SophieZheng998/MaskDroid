...
class PreModel_RUN(AbstractRUN):
    ...
    def evaluation(self, args, data, model, epoch, base_path, name = "valid"):
        ...
        with torch.no_grad():
            for batch in eval_loader:
                ...
                predictions, logits_cl = model.evaluate(batch.x, batch.edge_index, batch.batch)
...
class PreModel(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        nn.init.xavier_uniform_(self.preprocess.weight)
        ...
        nn.init.xavier_uniform_(self.graph_pred_linear_v2.weight)
        ...
        #self.graph_pred_linear.apply(init_weights)
    ...
    def forward(self, x, edge_index, batch, y):
        ...
        cl_loss = self.lambda_cl * torch.sum(cosine_sim_benign) + self.lambda_cl * torch.sum(cosine_sim_mal)
    ...
    def evaluate(self, x, edge_index, batch):
        ...
        predicted_cl = torch.logical_not(mask).int()
        ...
        return predicted_labels, logits_cl, logits_mlp
    ...
    def __init__(self, args, in_channels, out_channels):
        ...
        for layer in range(self.num_layers):
            if layer == 0:
                ...
                self.convs.append(self.gnn_layer(self.gnn_type, self.hidden_channels, self.out_channels))
