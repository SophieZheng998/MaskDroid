...
class JSMA_gnn_v2(JSMA):
    ...
    def compute_jacobian_self(self, x_features, edge_index, api_batchs, y, steps=5):
        ...
        for new_adj in scaled_inputs:
            ...
            self.model.convs[0].cached = True
            ...
            for i in range(y_score.shape[1]):
                if self.model.convs[0]._cached_edge_index[0].grad is not None:
                    self.model.convs[0]._cached_edge_index[0].grad.zero_()
            ...
            self.model.convs[0]._cached_edge_index = None
            self.model.convs[0].cached = False
