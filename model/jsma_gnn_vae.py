...
class JSMA_gnn_vae(JSMA):
    ...
    def label_consistency(self, apk, target_label):
        ...
        #adj = adj.half()
        y_pred, logits_mlp  = self.model.evaluate(apk.x, adj, apk.batch)
        #y_pred = logits.argmax(dim=1)
        #y_pred = torch.max(y_pred)
        
        return y_pred == target_label, [logits_mlp]   
    ...
    def compute_jacobian_self(self, x_features, edge_index, api_batchs, y, steps=10):
        ...
        for new_adj in scaled_inputs:
            ...
            for i in range(y_score.shape[1]):
                ...
                adj_grad = self.model.pretrain_vae.convs[0]._cached_edge_index[0].grad.t().clone()
