...
class JSMA():
    def __init__(self, model, logger:Logger, substitute_model=None, steps=10, model_type='msdroid'):
        """
    ...
    def attack(self, test_data):
        ...
        for i, apk in enumerate(loader):
            ...
            adversarial_sample, false_negative, iter, graph_too_large = self.perturb_single(apk, target_label)
    ...
    def perturb_single(self, apk, target_label):
        ...
        num_features = num_node * num_node
        if num_features > 10000000:
            graph_too_large = True
            return adversarial_sample, false_negative, iter, graph_too_large

        while iter < self.max_iters:
            # Jacobian matrix of forward derevative
            integrated_grad_list, search_space = self.compute_jacobian(var_apk.x, var_apk.edge_index, var_apk.batch)
    ...
    def compute_jacobian(self, x_features, edge_index, api_batchs, steps=10):
        ...
        for new_adj in scaled_inputs:
            ...
            y_score = self.substitute_model(x_features, new_adj, api_batchs)
    ...
    def label_consistency(self, apk, target_label):
        if self.model_type == "msdroid":
            # prediction results for all apis in an apk (apk.batch is batch indexes for all apis)
            ...
            y_score = self.model(apk.x, adj, apk.batch)
