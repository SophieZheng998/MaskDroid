...
class JSMA_new(JSMA):
    ...
    def label_consistency(self, apk, target_label):
        ...
        adj = adj.coalesce()
        y_pred, logits_cl, logits_mlp  = self.model.evaluate(apk.x, adj, apk.batch)
        #y_pred = logits.argmax(dim=1)
        #y_pred = torch.max(y_pred)
        
        return y_pred == target_label, [logits_cl, logits_mlp]
    ...
    def compute_jacobian_self(self, x_features, edge_index, api_batchs, y, steps=10):
        ...
        for new_adj in scaled_inputs:
            ...
            _, y_score_cl, y_score_mlp = self.model.evaluate(x_features, new_adj, api_batchs)
            y_score = y_score_cl + y_score_mlp
